...
The current source graph is in [SOURCE_MAP.md](docs/SOURCE_MAP.md); the
replacement contracts and rationale are in [ARCHITECTURE.md](docs/ARCHITECTURE.md).
YTP/1 is the implementation being completed. Transport v2 remains runnable as
a development reference and a source of useful components. Its default build
does not require finishing its audit backlog, tuning it, or porting every
historical feature before replacement work. Qualify the required behavior
before retiring its only working implementation.
