...
Per-key metadata accepts only the modular `permissions.allow_codecs` spelling.
Unknown or misplaced fields, including `allow_monero_rpc`, are rejected. The
server configuration file is a different document; it takes
`allow_monero_rpc_codec` or `allow_codecs`.
...
- `allow_codecs` does not imply `allow_local_ip` or `control_full`.
