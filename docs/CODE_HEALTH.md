...
| YH-005 | Medium | Anonym refresh lifetime | Resolved | A scope-owned `std::jthread` requests stop, wakes its wait, and joins during every stack-unwind path. |
...
| YH-011 | Low/medium | Large coordinator files and dead crypto surface | Partial | The static-Linux curl process/private-file boundary is extracted from the anonym proof coordinator with focused argument, environment, descriptor, permission, and cleanup tests. ABI, H2, relay-runtime, and remaining CLI decomposition plus unreachable legacy-crypto removal remain open and require their own reachability/compatibility tests. |
