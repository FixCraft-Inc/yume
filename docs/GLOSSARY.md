...
| Transport v2 | The previous protocol, kept in the uninstalled reference programs while its remaining features move to YTP/1. |
| YTP/1 | YUME Transport Protocol 1, which `yume` and `yumed` speak. |
...
| Execution context | The single event loop that runs one endpoint's network I/O and callbacks in order. |
