...
At startup `yumed` refuses a program that is a symbolic link or not a regular
file, belongs to a user other than root or the daemon's, is writable by group
or others, is not executable, is set-user-ID or set-group-ID, or has file
capabilities. SIGHUP reloads credential stores only, so a changed module
