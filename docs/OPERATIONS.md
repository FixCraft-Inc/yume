...
The server's `limits.max_egress_mbps`, from 1 to 1000000 megabits per second,
caps the stream payload the daemon relays. Each byte counts once, in either
direction and for every service. Protocol framing and control messages do not
count. Clients share the rate by weight. A busy client gets its entry's
`weight`, a number from 0.1 to 100 that defaults to 1, divided by the total
weight of the clients sending at that moment, and a client that stops sending
leaves its share to the others. When another client starts sending, transfers
already scheduled keep their earlier share, so the total can exceed the rate
until they have started. Without the field the daemon does not limit the rate
and weights have no effect.
...
sessions use the new grants for their next stream and a changed weight for
their next transfer. Sessions of a removed
...
A server `module` adapter runs a program for one stream service, as
[modules](MODULES.md) describes. The program runs as the daemon's user and can
read the daemon's credentials, so configure only programs you trust with them.
Under the packaged service a module shares the daemon's control group and
sandbox: the unit's memory and task limits count it, and it can write only
where the daemon can. The daemon logs each start and exit under the service
name, as in `module echo: exited with exit status 1, restarting in 1000 ms`.
...
A numeric host or `endpoint.connect_address` needs no lookup. A client with
`endpoint.socks5_proxy` looks nothing up either: it connects to the proxy's
numeric address and the proxy resolves the server's name. A client with a
managed TUN adapter needs a numeric first hop, the proxy or else the server,
so that the route to it can be excluded before the tunnel changes routing. Every address a destination name resolves
