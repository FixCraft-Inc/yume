...
The GUI source in `src/gui` does not build until it has a native interface,
so no package carries it.
...
supports named packet channels and routed TCP/UDP OPEN. Clean-prefix CMake and pkg-config
fixtures run when SDK installation and tests are enabled.

## Debian packages and service

The Debian source metadata selects the native application and leaves out the
shared ABI, the experimental SDK, the GUI and the BaseFWX modules. It declares
two packages:
...
dropping other clients. The server serves its cover site in-process.
