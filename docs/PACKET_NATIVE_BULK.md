...
`yumed-v2-reference` deliberately does not run firewall or routing commands.
Linux administrators who want a `wg-quick`-style setup can use the separate,
explicit `tools/yume_packet_quick.py` helper from the source tree. Review its
complete plan first:
