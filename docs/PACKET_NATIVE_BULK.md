...
and `yume_packet_write_batch` / `yume_packet_read_batch`. Traffic generated by
`iperf3` through this interface therefore measures the public ABI copies,
packet batching, authenticated carrier, server packet egress, and kernel TUN
path. It is not a synthetic stream benchmark.

Build it with the shared ABI and tools enabled:

```bash
cmake -S . -B build-abi-tools \
  -DCMAKE_BUILD_TYPE=Release \
  -DYUME_BUILD_SHARED_ABI=ON \
  -DYUME_BUILD_TOOLS=ON
cmake --build build-abi-tools --target yume-abi-tun yumed -j2
```

Prepare the server interface and run packet-enabled `yumed` as described
above. For the default CIDR, assign the reserved operator side and bind an
`iperf3` server to it:

```bash
sudo ip tuntap add dev yume-pkt0 mode tun user "$USER"
sudo ip addr add 10.89.0.1/24 dev yume-pkt0
sudo ip link set dev yume-pkt0 mtu 1420 up

iperf3 -s -B 10.89.0.1
```

On the client, create a separate persistent TUN, start the ABI adapter, and use
the IPv4/MTU printed in `packet_status` (normally `10.89.0.2/24` and 1420 for
the first client):

```bash
sudo ip tuntap add dev yume-abi0 mode tun user "$USER"
build-abi-tools/bin/yume-abi-tun \
  --config config/yume.json --tun yume-abi0

# In another terminal, using the values printed above:
sudo ip addr add 10.89.0.2/24 dev yume-abi0
sudo ip link set dev yume-abi0 mtu 1420 up
iperf3 -c 10.89.0.1
iperf3 -c 10.89.0.1 -R
```

Run the same commands through `yume --packet-tun yume-cli0` on a separately
created interface to compare the CLI adapter. Keep payload, MTU, `iperf3`
options, direction, server, carrier build, and physical route identical. Do
