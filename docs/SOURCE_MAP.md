...
| `modules/` | Module programs, which link no YUME library, and the libraries they build on. `echo/` is the example module that the tests run. `relay/` is the relay v2 end-to-end channel between two users (handshake, records and hybrid ratchet) and the stores its users keep (encrypted chat history, the password-derived relay secret and peer trust), built with `YUME_BUILD_BASEFWX_MODULES` for the planned chat and file modules. `files/` holds relay file transfer on top of it: an outbound source pinned to one opened file and a confined receiver that publishes only a verified file |
