...
| `facade/` | `yume_embed` (config marshalling, in-process client, log ring, and the transport-v2 backend behind `session/endpoint_backend.hpp`) and `yume_facade` (desktop session objects, traffic meter, key management) |
...
`yume_ytp1_h2_carrier` and the remaining transport-v2 `yume_core` link it.
...
| H2 carrier | A 12-byte length envelope preserves each record across partial reads. The shared H2/WebSocket implementation carries it through RFC 8441 extended CONNECT and accounts for receive credit. | `providers/ytp1_h2_carrier.cpp`, `core/stealth/h2_carrier.cpp`, `core/stealth/websocket_codec.cpp` |
...
`yume_private_files` is the shared owner of `core/security/secret_file.cpp`.
