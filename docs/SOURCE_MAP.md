...
| Path | Owns |
| --- | --- |
| `abi/` | The opt-in C ABI. `yume_c.cpp` owns handles, input validation, diagnostics, callback rules and backend leasing. It parses schema 1 through `config/v1` and reaches the runtime only through the embedding seam `endpoint_backend.hpp`. `ytp1_backend.cpp` is `yume_embed_ytp1`, which runs `NativeEndpoint` on its own execution thread and links no BaseFWX. Without the native provider graph, schema-1 start is unsupported. `compatibility_manifest.hpp` is the version view the ABI reports. |
...
| TLS profile | `providers/ytp1_tls13_secure_channel.cpp` applies `stealth/tls_client_profile.*`, shared with transport v2; a published YTP channel still requires TLS 1.3 and H2 | Qualify actual provider output and full-session behavior; native profile configuration alone does not establish circumvention |
| Session composition | `runtime/native_endpoint.cpp` calls `SessionBootstrap::create` for both roles, retains active engines and composes configured direct TCP/UDP adapters whose schema-1 destinations `runtime/native_egress_policy.*` enforces | `abi/ytp1_backend.cpp` drives named streams/packets and routed client OPENs for the ABI; standalone application-service adapters remain open |
...
omits wall time and evidence-profile IDs. `providers/ytp1_front_door.*`
...
`providers/ytp1_cover_site.*` loads an immutable operator-supplied static site
...
| Record protection | The security provider seals the canonical frame with AES-256-GCM. A directional epoch and sequence identify a one-use key/nonce pair. The engine prefixes the ciphertext with that token. | `engine/session_engine.cpp::protect_frame`, `providers/ytp1_security_provider.cpp` |
| H2 carrier | A 12-byte length envelope preserves each record across partial reads. The shared H2/WebSocket implementation carries it through RFC 8441 extended CONNECT and accounts for receive credit. | `providers/ytp1_h2_carrier.cpp`, `stealth/h2_carrier.cpp`, `stealth/websocket_codec.cpp` |
| Outer connection | OpenSSL TLS 1.3 encrypts H2 bytes. Memory BIOs exchange encrypted bytes with a ByteChannel. The Asio client provider reads and writes the TCP socket. | `providers/ytp1_tls13_secure_channel.cpp`, `providers/asio_tcp_byte_channel_provider.cpp` |
...
| --- | --- |
| TLS 1.3 | `ytp1_tls13_secure_channel.cpp` verifies the configured trust and hostname and selects H2 with ALPN. Distinct TLS exporter labels bind YTP authentication and H2 admission to this connection. Its client applies the shared browser profile; full-session stealth remains unqualified. |
| Ed25519 + ML-DSA-87 | `ytp1_security_provider.cpp::verify_composite` requires both signatures for the composite identity. The client checks its configured server identity. The server selects an authorized client identity and its access PSK. |
...
| Embedding YUME | `include/yume/yume.h`, [ABI.md](ABI.md), `abi/stream_integration_probe.c`, `abi/ytp1_stream_probe.c` |
...
transport v2 and `yume_abi_ytp1_stream_integration` for schema 1. Each
