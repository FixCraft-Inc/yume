...
| `common/` | Dependency-pure contracts shared across replacement layers and the ABI: the canonical service-name grammar and bound, canonical destination networks, and the egress address classes that configuration and native policy share |
...
| `runtime/` | `native_credentials.*` loads protected schema-1 files and per-identity service policy; `native_endpoint.*` owns native provider/bootstrap/session composition and optional automatic server accepts, paced by the dependency-pure `accept_scheduler.hpp`. `native_egress_policy.*` evaluates configured direct-adapter destinations |
...
| --- | --- | --- |
| Server ingress | `providers/ytp1_front_door.*` implements native listening, configured static cover, exporter/replay admission and retained H2/TLS promotion | Standalone runtime integration through `runtime/native_endpoint.*`. The experimental schema-1 ABI backend already composes it |
| Client admission | `providers/ytp1_h2_carrier.*` owns a 32-byte key and emits a fresh exporter-bound proof through `ytp1_h2_admission.*` | Composed by `runtime/native_endpoint.*` and reached through the schema-1 ABI backend. Standalone adapters remain |
...
| Applications | Existing client/daemon and adapters use the transport-v2 graph | Thin replacement CLI/runtime and required SOCKS, service and packet/TUN paths |
...
`AsioTcpAcceptedChannelOwner` adopts already-connected sockets using the same
bounded channel state and registry as client TCP. It supplies no listener or
cover policy. The FrontDoor uses it for every accepted socket, settles its
...
schema-1 ABI backend supplies named byte streams to it. Standalone adapters must
still supply bytes or packets to the engine.
