...
| `providers/` | Opt-in OpenSSL security, internal deterministic `ytp1_crypto` constructions, TLS channels, Asio TCP/UDP routes and client/accepted TCP channels, H2 carrier, native FrontDoor and configured static cover. `loopback_http_fetch.*` fetches GET and HEAD from a loopback web server for a reverse-proxy cover. `child_process.*` starts a helper program with one passed descriptor and watches it through a pidfd, and `process_privileges.*` drops a helper's privileges before it works |
