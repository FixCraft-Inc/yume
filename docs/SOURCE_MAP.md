...
| Applications | The default client/daemon use the transport-v2 graph. Development `yumed-ytp1`/`yume-ytp1` run direct routes and SOCKS5 CONNECT over `NativeEndpoint` | SOCKS5 UDP, named-service and packet/TUN adapters, qualification and the coordinated primary switch |
...
adapter supplies CONNECT byte streams. Packet adapters must still supply packets
to the engine.
