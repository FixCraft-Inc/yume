...
- **Real-browser GREASE values** (RFC 8701) in the ClientHello cipher suites / supported_groups / extensions / ALPN lists. Chrome and Firefox emit GREASE values from `{0x0A0A, 0x1A1A, …, 0xFAFA}` at position 0 of those lists; stock OpenSSL doesn't, so the compiled-in browser profiles omit them. The JA3 hash itself ignores GREASE (per the JA3 spec, GREASE values are dropped before hashing) — so yume's JA3 hashes still match the canonical Chrome / Firefox cluster baselines. A more sophisticated classifier (Hyperion / Suricata's TLS plugin, or any DPI engine that explicitly checks for "GREASE present in cipher list as a Chrome-likeness signal") will see the difference. Two viable approaches, both on the post-1.x roadmap:
    - **Custom BIO wrapper** (~400–700 LOC): wrap the underlying socket BIO with one that intercepts the first `BIO_write` carrying a TLS handshake record (`type == 0x16`, handshake `type == 0x01 ClientHello`), parses the ClientHello, injects one GREASE per list at position 0, rewrites the record / handshake / list lengths, and passes the modified bytes through. Subsequent writes pass through unchanged. The existing JA3 self-check in `tls_stealth.cpp` already demonstrates the BIO-mem capture pattern (lines 660–706) — start there. Gotchas: OpenSSL can chunk the handshake write across multiple `BIO_write` calls (rare but possible); the parser must handle both monolithic and fragmented cases.
    - **BoringSSL-bindings backend** (days, multi-commit): swap OpenSSL for BoringSSL at the link layer and use `SSL_set_grease_seed` / `SSL_CTX_set_grease_extensions` directly. BoringSSL exposes a mostly-compatible OpenSSL C API but with subtle differences `boost::asio::ssl::stream` may not handle; expect to need a vendored boost::asio patch or a port to the lower-level boost::asio socket APIs.
    
    Acceptance criterion for either approach: after the change, a `tcpdump` of a fresh yume connection contains a GREASE value at index 0 of the cipher_suites list (parse with `tshark -V -O tls` and look for "GREASE"). JA3 baselines pinned in `main_server.cpp` should NOT change (the JA3 hash drops GREASE before computing) — if they do, the rewrite is shifting non-GREASE positions and is wrong.
