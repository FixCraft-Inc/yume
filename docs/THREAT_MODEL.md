...
modules and packet adapters. Federation and host-controller modes left with
transport v2, and native YTP/1 has no command execution. Chat and file relay
are planned as modules on the relay channel library, and the GUI waits for a
native interface. Each needs its own design before it ships. See
