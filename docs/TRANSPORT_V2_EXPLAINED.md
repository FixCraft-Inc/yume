<!-- Generated from docs/src/en_US/pages/transport_v2_explained.doc by scripts/yume_docs.py. Edit that file, not this one. -->
# Transport v2 explained

This page describes the transport-v2 reference programs, `yume-v2-reference`
and `yumed-v2-reference`. They are built only with `YUME_BUILD_TRANSPORT_V2=ON`,
are not installed, and keep capabilities that YTP/1 has not taken over yet.
[YUME explained](EXPLAINED.md) covers the default programs.
[How a byte travels](BYTE_PATH.md) walks through this stack one layer at a
time.
