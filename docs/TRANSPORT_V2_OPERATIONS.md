<!-- Generated from docs/src/en_US/pages/transport_v2_operations.doc by scripts/yume_docs.py. Edit that file, not this one. -->
# Transport-v2 reference operations

These are the operator instructions for the explicitly enabled transport-v2
reference programs, `yume-v2-reference` and `yumed-v2-reference`. They are not
installed by default and their configuration and flags do not apply to the
native `yume` and `yumed`, which [operations](OPERATIONS.md) covers. The
reference graph remains available while its capabilities migrate. See
[implementation status](IMPLEMENTATION_STATUS.md) for that boundary.
...
ExecStart=/usr/local/bin/yumed-v2-reference --listen 443 --cert /etc/yume/server.crt --key /etc/yume/server.key --auth-keys /etc/yume/authorized_keys --obfs-secret-file /etc/yume/secrets/admission.hex --inner-psk-file /etc/yume/secrets/inner.hex --real-backend loopback://127.0.0.1:3000 --real-index /etc/yume/cover-index.html
Restart=on-failure
RestartSec=3
...
loopback address. Start and health-check it before `yumed-v2-reference`; do not expose its
...
## Transport-v2 configuration loading and saving

Client and server CLI/facade readers reject unknown keys, explicit `null`,
and wrong scalar types. Omitted fields keep initialized defaults or CLI
overrides. Parsing,
validation, and startup are separate gates: a value that fits its storage type
can still be refused by the transport profile or by a startup resource limit.
Schema 1 belongs to the separate YTP/1 path.

| Field or operation | Current contract |
| --- | --- |
| `obfs_pad_multiple`, `obfs_jitter_ms` | Default `0`. Successful load/save preserves the values. Client and server profile validation requires both to be `0`; saving a configuration does not enable shaping. |
| Server `auth_keys_meta`, `upstream_response_dir` | Nonempty relative paths resolve beside the configuration file, or against the explicit base directory for an in-memory facade parse. Empty paths stay empty. Metadata policy is loaded under the authorization snapshot transaction at startup/reload. |
| Server `real_root`, `real_backend` | A nonempty value enables `real_http`, including when `real_http:false` is also present. Empty fields do not enable it. The backend still needs a separate static or captured cover source for probe paths. |
| Server `pq_auto_generate`, `allow_embedded_master` | JSON booleans only, including when a CLI override supplies the effective value. Parsing `pq_auto_generate:true` does not implement automatic key generation. |
| Client `tls_pin`, `tls_pin_sha256` | Both must be strings when present. The canonical `tls_pin` wins, including an empty string. The Android writer still uses `tls_pin_sha256`; native writers emit `tls_pin`. |
| Client `app_codec_listen` | A malformed combined endpoint is a load error even when a CLI listener override is present. Split host/port fields, when supplied, take precedence after the combined endpoint parses. Native saves emit the combined endpoint without stale split fields. |
| Client `--save-server` | Saves effective values, including automatic thread/control-port selection, one tunnel, disabled SOCKS/shaping, and cleared identity, trust and storage references. Previous values cannot survive by being omitted from an update. |

Failed CLI loads preserve the supplied configuration and, for the daemon,
its path-resolution context. Facade parsers return no candidate on failure.
Serialization completes before atomic file replacement, so a serialization
failure preserves the existing file. The client string serializer also
preserves its caller's output on failure. These publication guarantees do
not imply that saving runs startup checks or validates referenced files.

The parsers retain separate field ordering and validation stages. A document
with several errors can produce a different first diagnostic in the CLI and
facade. Use the [ABI's typed result and JSON pointer](ABI.md#strict-configuration)
where available rather than interpreting diagnostic text as a status code.
...
```bash
tools/yume_packet_quick.py up --listen 192.168.1.10:8443 \
  --allow-from 192.168.1.0/24 --dry-run
sudo tools/yume_packet_quick.py up --listen 192.168.1.10:8443 \
...
`yumed-v2-reference` itself never invokes it.
...
```bash
yumed-v2-reference --auth-keys /etc/yume/authorized_keys --keys-list
```

Add a generated key:

```bash
yumed-v2-reference --auth-keys /etc/yume/authorized_keys --keys-gen ./keys/user1 --keys-gen-add
```
...
```bash
yumed-v2-reference --auth-keys /etc/yume/authorized_keys --keys-remove alice
```
...
- confirm no HTTP proxy is terminating TLS in front of `yumed-v2-reference`
...
- verify the separately supervised Node cover is listening on the configured
  loopback IP and passes `yumed-v2-reference`'s startup health check
