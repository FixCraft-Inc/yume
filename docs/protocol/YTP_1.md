...
YTP/1 kernel, session engine, native providers, and internal native endpoint.
...
Admission proves possession of the separate 32-byte admission key before the
server starts YTP AUTH. It does not authenticate a transport identity or grant
...
`/`, and 64 lowercase hexadecimal nonce characters. No query, trailing data,
uppercase hex or alternative encoding is accepted. The nonce is 32 fresh
cryptographically random bytes for each client carrier creation.

The HMAC-SHA256 key is exactly 32 bytes. Its canonical input is:

```text
u16 domain_length
u8  domain[domain_length] = "yume/ytp/1/h2-web/admission/v1"
u16 server_name_length
u8  canonical_server_name[server_name_length]
u16 exporter_length = 32
u8  exporter[32]
u8  nonce[32]
```

The server name is lowercase ASCII DNS text, at most 253 bytes, with nonempty
labels of at most 63 bytes, no trailing root dot, and no leading or trailing
hyphen in a label. The client uses the same name its TLS provider authenticated;
the server checks HTTP authority against received TLS SNI and its listener port.
Both obtain 32 exporter bytes locally from the completed TLS connection using
`EXPORTER-yume/ytp/1/h2-web-admission/v1` and an empty context. The exporter is
never a peer-supplied field. Token comparison is constant time.
...
monotonic time; duplicates, saturation, clock regression, overflow and failed
allocation refuse admission without evicting a live nonce. At most one
successful carrier promotion is allowed for a TLS connection throughout its
lifetime. A TTL cache alone cannot enforce that rule after an entry expires.
Missing, invalid, replayed or resource-exhausted admission follows the genuine
configured cover path and never starts AUTH. The opt-in native FrontDoor
implements these ingress rules with a configured immutable static website;
they do not establish a complete YTP session runtime.
...
Both per-stream receive windows begin at zero. Sending OPEN does not accept
it and the opener MUST NOT send a stream-credit update, DATA, or PACKET until
it receives the receiver's first STREAM_CREDIT. The receiver MUST first
complete per-OPEN authorization and handler acceptance; a destination-bearing
...
write within that reverse-direction grant. Pending OPENs count against the
session's pending-open and stream bounds until accepted or refused. Local
cancellation before acceptance sends a terminal CLOSE and completes that
pending OPEN once; crossed acceptance cannot revive it. If OPEN is still
unpublished behind the outbound rekey barrier, cancellation drops that OPEN
without sending CLOSE for a stream the peer has never observed.
...
| 5 | settled | Ordered confirmation that the sender will publish no further records for this stream. |

Code 0 closes only the sender's write side. Already accepted inbound records
remain readable. After both directional FINs and draining queued inbound data,
each side sends code 5 and retains the stream's closing state until receiving
the peer's code 5. Returning application-held credit after closure restores
connection credit only.

Codes 1 through 4 terminate both directions and fail pending OPEN and I/O.
The receiver sends code 5 after settling application ownership. The initiator
retains the closing entry until that acknowledgement arrives. If terminal
CLOSEs cross, each side sends code 5 and waits for the other's code 5.
A side sends at most one settlement acknowledgement for each stream.

A terminal CLOSE or code 5 MUST follow every already-published record for that
stream in the ordered record direction; no later DATA, PACKET or STREAM_CREDIT
may be published in that direction. While waiting for the peer barrier, a
locally closed stream validates crossed records against its service kind and
remaining granted credit. Crossed DATA/PACKET is discarded and returns only
connection credit; crossed stream credit remains bounded and cannot reopen
the stream. Closing entries count against the session stream bound, so a peer
that withholds settlement cannot cause unbounded tombstone retention.

An unknown stream ID, a repeated FIN, an unexpected or repeated settlement,
an unknown close code, data after the peer's FIN, or DATA, PACKET or
STREAM_CREDIT after the peer's terminal barrier is a protocol failure. A
required crossed settlement acknowledgement remains valid after terminal CLOSE. Refusal does not invalidate
other streams or prevent a subsequent authorized OPEN on the same session.
...
preserves TLS/H2 state, provider provenance and executor affinity after cover
output and timer drain. Ordinary H2 cover handling remains with the carrier.
The caller owns execution and completion drain; this ingress performs no DNS.
...
half-close and final ownership drain. This is an internal runtime seam; its
tests do not establish a complete runnable tunnel or production qualification.
