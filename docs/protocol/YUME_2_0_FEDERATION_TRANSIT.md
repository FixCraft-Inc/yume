...
The one-hop federation fixtures were removed with the transport-v2 runtime.
`tests/test_yume_federation_v2.py` exercised one federated relay-v2 DATA/CLOSE
transfer between two directly peered daemons, and
`tests/test_yume_federation_cluster.py` required each end of a three-node line
to neither learn nor address an endpoint two links away. Both remain in Git
history.
[`src/modules/relay/directory_policy_test.cpp`](../../src/modules/relay/directory_policy_test.cpp)
still pins the raw and one-prefix directory grammar.
