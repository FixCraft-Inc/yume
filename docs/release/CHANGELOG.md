...
- **Evaluation tooling.** `scripts/ensure-ndpi.sh` builds pinned nDPI 6.0,
  `scripts/yume_ndpi_smoke.py` records nDPI output for one rootless loopback
  session, and `scripts/yume_ethernet_smoke.py` runs a direct-link session after
  a preflight that fails when the link is unavailable. Their results are
  observations, not classifier verdicts or benchmarks.
