...
The product version moved to `0.3.0-dev1`. The default `yume` and `yumed`
binaries now compose native YTP/1 with schema-1 configuration. The explicit,
uninstalled transport-v2 reference retains its wire `0.2.0-dev6`; the wire,
AUTH, relay, YTP/1, schema, ABI, and evidence-profile
...
- **Relay channel library.** The relay v2 end-to-end channel between two users
  ...
  the transport-v2 code. It also holds the encrypted chat history, the relay
