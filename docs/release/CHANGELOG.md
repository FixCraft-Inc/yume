...
The product version moved to `0.3.0-dev1`. `yume` and `yumed` compose native
YTP/1 with schema-1 configuration, and transport v2 has been removed. The
YTP/1, relay channel, schema, ABI and evidence-profile versions are
independent axes. `docs/IMPLEMENTATION_STATUS.md` is the current
...
- **Relay channel library.** The relay v2 end-to-end channel between two users
  ...
  and wire format are unchanged, which tests check against known answers from
  the transport-v2 code and against an independent Python reference that pins
  every handshake and ratchet label. It also holds the encrypted chat history, the relay
  secret, peer trust, and the invite and directory records with their
  policies. Peer fingerprints changed from the transport-v2
  identity domain to the YTP/1 one, so a peer's fingerprint is the one
...
- **SOCKS5 upstream provider.** `providers/socks5_upstream.*` reaches a server
  ...
  reply. A client configuration's `endpoint.socks5_proxy` names the proxy's
  numeric address and port and an optional protected credentials file with a
  username line and a password line, and a managed TUN excludes the proxy's
  route from the tunnel.
...
- **Egress lists.** Server `direct_tcp` and `direct_udp` destinations can add
  ...
  Lists refuse more but never permit more than `public` and `networks`. This
...
- **Modules.** A server `module` adapter runs a program for one stream
  ...
  and restarts the program with backoff when it exits. The program runs
  without capabilities under no_new_privs, and the kernel kills it if the
  daemon dies. It still runs as the daemon's user and can read the daemon's
  files, so only trusted programs belong there. `yume-doctor` checks the
  adapter, and `yume-echo-module` is an example. See
  [modules](../MODULES.md).
- **Local forwards.** A client `forward` adapter listens on a loopback TCP
  port or a UNIX socket and turns each connection into a stream on its
  service, optionally to a fixed destination that the server's `direct_tcp`
  policy checks. This replaces transport v2's `-L` forwards. Their TCP
  listeners were not limited to loopback, while native forwards are. A UNIX
  socket only accepts the client's own user. UDP forwards are not ported yet.
...
- **Bounded name-lookup shutdown.** Client hosts and destination names now
  ...
  longer delays other lookups, and closing a runtime kills and reaps the helper,
  so SIGTERM or SDK stop no longer waits for a stalled `getaddrinfo`. The helper
  holds no inherited connections and drops every capability. `yume` and `yumed`
  serve as their own helper. The SDK installs `yume-resolver`, which embedding
  hosts pass as `yume_runtime_options.resolver_program`. Without it, a client
  that dials a name fails closed at start.
- **Per-identity session limits.** An authorized-keys entry may set
  `max_sessions`. A new session beyond it replaces that identity's oldest
  session, so a reconnecting client displaces its own half-open session
  instead of being refused. Without the field the daemon's capacity is the
  only bound, as before. Doctor validates the field against the daemon's bound.
- **Credential reload.** `yumed` reloads its key stores and server keys on
  SIGHUP, and the Debian unit wires `systemctl reload`, validating first.
  Removed identities lose their sessions at once, established sessions apply
  changed grants to their next stream, and an invalid store keeps the previous
  credentials. TLS and admission key changes still need a restart.
- **More client bundles.** `yume-setup add-client` issues another client for an
  existing server tree and extends its authorized-keys store in one
  all-or-nothing step that keeps the store's file owner. `remove-client`
  revokes one, and a reload ends its sessions.
- **Client status for embedding.** `NativeClientRuntime::status()` reports the
  connection state, last failure, verified server fingerprint, attempt counts
  and cumulative traffic from any thread, with an optional state-change
  callback. `SessionEngine::traffic()` supplies per-session byte counts.
- **Native operations guide.** `docs/OPERATIONS.md` now covers the native
  programs. The transport-v2 operator material moved to
  `docs/TRANSPORT_V2_OPERATIONS.md` under its reference program names.
  Release verification, which describes the native release, stays on the
  native page. The duplicate website build notes were dropped in favor of the
  documentation source guide.
- **Native acceptance tooling.** Native-only builds register setup, doctor and
  packaging regressions. Bounded libFuzzer targets cover YTP framing, AUTH and
  schema-1 configuration; numeric JSON overflow now reports a typed validation
  error. Optional isolated network tests exercise the actual systemd-resolved
  service and competing streams under kernel-induced latency and backpressure.
- **Cryptographic construction vectors.** A public synthetic corpus and an
  independent generator cover transcripts, key schedules, record protection
  and directional rekey calculations. The internal crypto module exposes those
  deterministic constructions to tests without exposing them through the SDK.
- **Native packaging migration.** Release archives and Debian definitions use
  schema-1 tools and configuration. GUI and helper build lanes explicitly select
  the retained reference graph. The Debian service waits for operator-provided
  credentials and validates configuration before starting; package installation
  does not enable or start it. Release qualification remains open.
- **Native terminal failure cleanup.** The CLI reserves its runner-exception
  stop task before execution. Client reconnect and SOCKS accept retry failures
  that cannot recover close the runtime and report a typed terminal status,
  allowing the process to drain and exit. Ordinary failures keep backoff.
- **TCP allocation-failure completion.** Pending read/write construction keeps
  callback ownership with the caller until storage exists. Allocation refusal
  settles each callback once and returns queue capacity for subsequent traffic.
- **Shared allocation test hooks.** The remaining ten private new/delete
  replacements use the shared out-of-line helper, including optional aligned
  provider allocation interception and secret-buffer quarantine. Fault policies
  and wipe-before-release observations remain owned by their tests.

- **Development standalone runtimes.** With every native provider enabled the
  default build installs `yumed` and `yume`. The daemon serves configured
  ...
  and requires clean SIGTERM exits. Managed Linux packet/TUN adapters and
  experimental SDK streams, packets and routes are also implemented;
  arbitrary standalone named-service handlers remain unfinished.
...
- **Runnable development kit.** Installed `yume-setup` and `yume-doctor` use
  schema 1. The generated kit starts native `yumed` and `yume` with SOCKS
  TCP/UDP. Managed TUN addresses, routes and DNS are explicit operator choices.
...
- **Evaluation tooling.** `scripts/ensure-ndpi.sh` builds pinned nDPI 6.0, or
  newer sources such as `--ref dev` recorded by commit. `scripts/yume_ndpi_smoke.py`
  records nDPI output for one rootless loopback session with Ethernet-sized
...
- **DNS route refusal.** Asynchronous OPEN permission failures now send the
  ...
  resolver answers, checks mixed allowed and denied answers in both orders,
  verifies that refused routes open no sockets, and carries traffic afterward.
- **Build and tooling portability.** The direct-route archive applies the shared
  ...
  new/delete call. The server teardown regression now uses those shared hooks
  too, retaining its caller-thread failure scope and all teardown checks.
...
- **The vendored nlohmann tree is complete, so CI builds again.**
  ...
  declares `basic_json` and `json_pointer` differently. Upstream publishes
  `json.hpp` and `json_fwd.hpp` as two standalone release assets and only the
  first was vendored, so the second is now vendored beside it, byte-identical
  to the upstream asset. Take the release asset rather than the multi-header
  distribution's file of the same name, which pulls
  `nlohmann/detail/abi_macros.hpp` from a tree this directory does not carry.
  `third_party/nlohmann_json/PROVENANCE.md` records both URLs and hashes, and
  a metadata test pins them and enforces that the vendored tree satisfies
...
- **Transport v2.** Its wire, AUTH v2, security modes, client and daemon, GUI
  ...
  build scripts, and their documents and manuals are gone. Commit `9070b0a` is
  the last that contains all of it. What YUME still needed moved first: the
