#!/usr/bin/env python3
"""Generate the immutable C++ transport-profile registry from evidence."""
...
def emit_profile(entry: dict[str, Any], index: int,
                 ids: set[str], aliases: set[str]) -> tuple[list[str], str]:
    ...
    require(isinstance(browser, dict) and isinstance(server, dict),
            f"{profile_id} manifest sections are incomplete")
...
def generate(registry_path: pathlib.Path,
    ...
    for index, entry in enumerate(entries):
        ...
        emitted, symbol = emit_profile(entry, index, ids, aliases)
...
def active_profile_metadata(
        ...
        generated_path: pathlib.Path = DEFAULT_OUTPUT) -> dict[str, Any]:
