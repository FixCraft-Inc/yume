...
class BrowserSandboxTest(unittest.TestCase):
    ...
    def test_isolated_wrapper_is_bounded_and_kills_children(self) -> None:
        outer = {
            "user": "user:[1]",
            "mount": "mnt:[2]",
            "pid": "pid:[3]",
    ...
    def test_isolated_workloads_drop_all_namespace_capabilities(self) -> None:
    ...
    def test_isolated_controller_requires_exact_single_id_maps(self) -> None:
    ...
    def test_direct_host_root_controller_fails_before_mount(self) -> None:
        reads = {
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "exact one-ID"):
                yume_bench_isolation.enter_isolated_controller({
                    "user": "user:[1]",
                    "mount": "mnt:[2]",
                    "pid": "pid:[3]",
                    "network": "net:[4]",
    ...
    def test_controller_accepts_distinct_uid_gid_maps_after_structural_checks(self) -> None:
        reads = {
        ...
        with (
            ...
            checks = yume_bench_isolation.enter_isolated_controller(outer)
        self.assertEqual(checks["single_id_uid_map"], 1000)
        self.assertEqual(checks["single_id_gid_map"], 2000)
        self.assertEqual(run.call_count, 3)

    def test_root_mount_must_not_be_shared_or_slave(self) -> None:
    ...
    def test_controller_requires_pid_1_before_mounts_or_network_queries(self) -> None:
        reads = {
            "/proc/self/uid_map": "0 1000 1\n",
    ...
    def test_fresh_network_namespace_rejects_preexisting_routes(self) -> None:
    ...
    def test_artifacts_are_forced_private(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_private_text_applies_mode_and_owner_before_write(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_output_owner_rejects_incomplete_sudo_identity(self) -> None:
    ...
    def test_artifact_mode_enforcement_rejects_symlinks(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "evidence"
            root.mkdir()
    ...
    def test_private_tree_cleanup_is_verified(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_source_provenance_binds_commit_dirty_state_and_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_source_provenance_rejects_parent_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repository = Path(tmp) / "repo"
            repository.mkdir()
    ...
    def test_runtime_security_record_is_fail_closed(self) -> None:
        state = {
        ...
        record = "YUME_BENCH_SECURITY_STATE=" + json.dumps(state)
        self.assertEqual(runtime_security_state(record), state)
        state["CapEff"] = "0000000000000001"
        bad_record = "YUME_BENCH_SECURITY_STATE=" + json.dumps(state)
        with self.assertRaisesRegex(RuntimeError, "not fail-closed"):
            runtime_security_state(bad_record)

    def test_node_sandbox_hides_secret_and_artifact_trees(self) -> None:
        node = yume_bench_isolation.FrozenExecutable(
    ...
    def test_frozen_node_is_sealed_before_version_execution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            node = Path(tmp) / "node"
    ...
    def test_exec_guard_validates_kernel_security_fields(self) -> None:
        status = "\n".join([
            "CapInh:\t0000000000000000",
            "CapPrm:\t0000000000000000",
            "CapEff:\t0000000000000000",
