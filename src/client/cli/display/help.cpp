...
void print_help() {
    std::cout << yume::release::render_brand_header(
        ...
        << "                             Without addr the bind is 0.0.0.0, every IPv4\n"
        << "                             interface and not only loopback.\n"
        ...
        << "  --monero-rpc-listen <addr:port>\n"
        << "                           Select the Monero codec and its loopback listener\n"
        ...
        << "  --configs <list|all>     Local benchmark config subset\n"
        << "  --list-configs           Print local benchmark configs and exit\n"
        << "  --duration-sec <N>       Full-mode duration hint (30..600)\n"
        << "  --latency-iters <N>      Local benchmark echo iterations\n"
        << "  --bulk-mib <N>           Local benchmark bulk size\n"
        << "  --streams <N>            Local benchmark concurrent streams\n"
        << "  --client-threads <N>     Local benchmark client threads (0 = auto)\n"
        << "  --server-threads <N>     Local benchmark server threads\n"
        << "  --cooldown-ms <N>        Pause between local benchmark configs\n"
        << "  --repeat <N>             Trials per local benchmark config\n"
        << "  --one-way                Measure local upload with sink/ack, not echo\n"
        << "  --json <path>            Write local benchmark JSON\n"
        << "  --json-stdout            Print local benchmark JSON after the table\n"
        << "  --keep-workdir           Keep local benchmark logs and generated keys\n"
        << "  --dev                    With --quick-bench/--full-bench, show raw\n"
        << "                             component tables and phase timings.\n"
        << "  --no-color               Disable colored benchmark grades.\n"
        << "  --color                  Force colored benchmark grades.\n"
        ...
        << "                           Long-form local forward (all three required)\n"
        << "  -R [bind:]rport:host:port\n"
        << "                           Reverse forward\n"
        << "  --run <cmd>              Run a command through Yume\n"
        << "  --proxycmd --dest <host> --dport <port>\n"
        << "                           One-shot SOCKS client for a proxy hook\n"
        << "  --exec <cmd>             Controlled-client EXEC request; currently denied\n"
        << "  --allow-exec             Unavailable; inbound execution stays disabled\n"
        << "  --control [id]           Control a registered client\n"
        << "  --id <id>                Controlled-client identifier for --control\n"
        ...
        << "  --accept-server-control [port]\n"
        << "                           Allow the server to open a bounded reverse SSH listener\n"
        << "  --accept-server-control-min-port <port>\n"
        << "  --accept-server-control-max-port <port>\n"
        << "                           Bound automatic reverse-listener selection\n"
        ...
        << "  --accept-monitoring      Pre-accept the privacy warning when proof is absent\n"
        ...
        << "  --password-stdin         For export/import, read the password from stdin;\n"
        << "                             for import, also skip the interactive write\n"
        << "                             confirmation\n\n"
        << "Other:\n"
        << "  completion bash\n"
        << "  --completion bash       Equivalent completion-script spelling\n"
        << "  -h, --help               Show help\n"
