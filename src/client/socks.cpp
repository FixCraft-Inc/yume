...
SocksSession::SocksSession(boost::asio::ip::tcp::socket socket, std::shared_ptr<Tunnel> tunnel)
...
void SocksSession::on_read_request_address(uint8_t atyp, const boost::system::error_code& ec, std::size_t) {
    ...
    if (atyp == 0x03 && awaiting_domain_len_) {
        ...
        boost::asio::async_read(socket_, boost::asio::buffer(addr_buf_),
                                boost::asio::bind_executor(strand_,
                                                           [self](const boost::system::error_code& e, std::size_t bytes) {
                                                               self->on_read_request_address(0x03, e, bytes);
                                                           }));
        return;
    }
...
SocksServer::SocksServer(boost::asio::io_context& io, int port, std::shared_ptr<Tunnel> tunnel)
    : acceptor_(io)
    , tunnel_(std::move(tunnel)) {
...
void SocksServer::do_accept() {
    acceptor_.async_accept([this](const boost::system::error_code& ec, boost::asio::ip::tcp::socket socket) {
        if (!ec) {
            auto session = std::make_shared<SocksSession>(std::move(socket), tunnel_);
            session->start();
        } else {
