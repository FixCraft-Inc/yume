...
#include "modules/codecs/codec.hpp"

namespace {

using yume::codecs::Bytes;
...
void test_rejects_nonzero_reserved_bytes() {
    ...
    payload[6] = 1;
    yume::codecs::Envelope envelope;
    std::string error;
    assert(!yume::codecs::decode_envelope(
...
void test_rejects_target_metadata_mismatch() {
    ...
    yume::codecs::Envelope envelope;
    std::string error;
    assert(!yume::codecs::decode_envelope(payload, 1024, &envelope, &error));
...
void test_rejects_target_injection_and_response_reason_injection() {
    ...
    yume::codecs::Envelope envelope;
    std::string error;
    assert(!yume::codecs::decode_envelope(request, 1024, &envelope, &error));

    yume::codecs::HttpResponse response;
    assert(yume::codecs::parse_http_response_head(
        "HTTP/1.1 200 OK\r\nX-Test: one\r\n\r\n", &response, &error));
    response.reason = "OK\r\nX-Injected: yes";
    const std::string built = yume::codecs::build_client_http_response(response);
    assert(built.find("X-Injected") == std::string::npos);
}

void test_strips_connection_nominated_headers() {
    yume::codecs::HttpRequest request;
    request.method = "POST";
    ...
    const auto built = yume::codecs::build_backend_http_request(
...
void test_registry_drives_every_codec_lookup() {
    const auto ids = yume::codecs::builtin_codec_ids();
    assert(!ids.empty());
    for (const auto& id : ids) {
        assert(yume::codecs::is_supported_codec(id));
        const auto descriptor = yume::codecs::builtin_codec(id);
        assert(descriptor.has_value());
        assert(descriptor->id == id);
        assert(yume::codecs::canonical_codec_id(id) == id);
        ...
        for (const auto& alias : descriptor->aliases) {
            assert(yume::codecs::canonical_codec_id(alias) == id);
            assert(yume::codecs::is_supported_codec(alias));
        }
    }
    assert(!yume::codecs::is_supported_codec("not-a-real-codec"));
    assert(!yume::codecs::builtin_codec("not-a-real-codec").has_value());
}

// The generic Endpoint must not carry any codec's defaults.
void test_generic_endpoint_has_no_codec_defaults() {
    const yume::codecs::Endpoint endpoint;
