...
bool DecodeRelayChunkBase64(std::string_view encoded,
    ...
    const std::optional<std::string> raw = relay::base64_decode(encoded);
    if (!raw || raw->empty() || raw->size() > max_decoded_bytes) {
...
bool RelayFileReceiver::Begin(
    ...
    std::unique_ptr<relay::identity::Sha256Stream> hasher;
    try {
        name.assign(basename);
        expected_digest.assign(expected_sha256);
        hasher = std::make_unique<relay::identity::Sha256Stream>();
