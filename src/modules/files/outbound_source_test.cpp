...
std::string ReadAll(const std::shared_ptr<yume::files::RelayOutboundSource>& source) {
...
int main() {
    ...
    std::string error;
    auto source = yume::files::RelayOutboundSource::Open(
        offered_path, offered.size(), &error);
    assert(source);
    assert(error.empty());
    assert(source->size() == offered.size());
    assert(source->Sha256Hex(&error) ==
           yume::relay::identity::sha256_hex(std::span<const std::uint8_t>(
    ...
    auto linked = yume::files::RelayOutboundSource::Open(
        offered_path, offered.size(), &error);
    assert(!linked);
    assert(!error.empty());

    auto oversized = yume::files::RelayOutboundSource::Open(
