...
using yume::relay::DirectoryNamespace;
using yume::relay::EndpointInfo;
...
nlohmann::json ValidEndpointJson(std::string id =
    ...
    return yume::relay::endpoint_to_json(
...
void CheckEndpointPolicy() {
    std::string error;
    auto endpoint = yume::relay::try_directory_endpoint_from_json(
    ...
    Check(!yume::relay::try_directory_endpoint_from_json(
    ...
    text_boundary["display_name"] = std::string(
        yume::relay::kMaxDirectoryDisplayNameBytes - 2U, 'n') + "\xc3\xa9";
    Check(yume::relay::try_directory_endpoint_from_json(
    ...
    for (std::size_t index = 0;
         index <= yume::relay::kMaxDirectoryRelationshipsPerList; ++index) {
    ...
    auto remote_json = yume::relay::endpoint_to_json(remote, true);
    Check(yume::relay::try_directory_endpoint_from_json(
    ...
    Check(!yume::relay::try_make_federated_visible_endpoint_id(
    ...
    Check(!yume::relay::try_make_federated_visible_endpoint_id(
               std::string(yume::relay::kMaxDirectoryFederationPeerIdBytes,
                           'p'),
               std::string(yume::relay::kMaxDirectoryEndpointIdBytes, 'e')),
          "oversized combined visible endpoint id was accepted");
}

void CheckServerIdentityPolicy() {
    using yume::relay::is_valid_directory_server_identity;
    ...
    Check(!is_valid_directory_server_identity(
               std::string(yume::relay::kMaxDirectoryServerIdBytes + 1U,
                           's'),
               "Yume", false),
          "oversized server id was accepted");
    Check(!is_valid_directory_server_identity(
               "server", std::string(
                   yume::relay::kMaxDirectoryServerNameBytes + 1U, 'n'),
               false),
...
void CheckResponsePolicy() {
    std::string error;
    const auto valid = yume::relay::try_directory_response_from_json(
    ...
    malformed["ok"] = 1;
    Check(!yume::relay::try_directory_response_from_json(
               malformed, DirectoryNamespace::ClientVisible),
    ...
    for (std::size_t index = 0;
         index <= yume::relay::kMaxDirectoryEndpoints; ++index) {
    ...
    const std::string maximum_identity(
        yume::relay::kMaxDirectoryIdentityBase64Bytes - 4U, 'A');
    malformed = ValidResponse();
    malformed["endpoints"] = nlohmann::json::array();
...
void CheckPresencePolicy() {
    std::string error;
    const auto valid = yume::relay::try_presence_announcement_from_json(
        ValidPresence(), &error);
    ...
    malformed["preferred_name"] = std::string(
        yume::relay::kMaxDirectoryDisplayNameBytes + 1U, 'n');
    Check(!yume::relay::try_presence_announcement_from_json(malformed),
    ...
    for (const auto& schema : {nlohmann::json(nullptr), nlohmann::json(1)}) {
        ...
        Check(!yume::relay::try_presence_announcement_from_json(malformed, &error) &&
