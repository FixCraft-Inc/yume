...
std::uint8_t EncodeChannelKind(ChannelKind kind) {
    switch (kind) {
...
ChannelKind DecodeChannelKind(const Bytes& value) {
    if (value.size() != 1) {
...
void ValidateRelayPsk(PasswordPolicy policy,
...
identity::CompositePublicKey ParseCanonicalIdentity(const Bytes& encoded) {
    if (encoded.empty() || encoded.size() > kMaxIdentityBytes) {
        throw Error("relay-v2 composite identity size is invalid");
    }
    identity::CompositePublicKey parsed =
        identity::parse_composite_identity(encoded);
    if (!parsed.valid()) {
        throw Error("relay-v2 composite identity is invalid");
    }
    const Bytes canonical = identity::encode_composite_identity(
...
Bytes EncodeUnsignedRequest(const HandshakeContext& context,
                            const Bytes& initiator_identity,
                            const Bytes& responder_identity,
                            const Bytes& initiator_mlkem_public_key,
                            const Bytes& initiator_x25519_public_key) {
...
Bytes EncodeRequest(const HandshakeContext& context,
                    const Bytes& initiator_identity,
                    const Bytes& responder_identity,
                    const Bytes& initiator_mlkem_public_key,
                    const Bytes& initiator_x25519_public_key,
                    const Bytes& signature) {
    if (signature.size() != identity::kCompositeSignatureLen) {
...
Bytes EncodeUnsignedResponse(const HandshakeContext& context,
                             const Bytes& initiator_identity,
                             const Bytes& responder_identity,
                             const Bytes& initiator_mlkem_public_key,
                             const Bytes& initiator_x25519_public_key,
...
Bytes EncodeResponse(const HandshakeContext& context,
                     const Bytes& initiator_identity,
                     const Bytes& responder_identity,
                     const Bytes& initiator_mlkem_public_key,
                     const Bytes& initiator_x25519_public_key,
...
struct ParsedRequest {
    ...
    Bytes signature;
    identity::CompositePublicKey initiator_public_key;
};

ParsedRequest ParseRequest(const Bytes& encoded) {
    ...
    if (request.signature.size() != identity::kCompositeSignatureLen) {
        throw Error("relay-v2 initiator signature size is invalid");
    }
    return request;
}
...
struct ParsedResponse {
    ...
    Bytes signature;
    identity::CompositePublicKey responder_public_key;
};

ParsedResponse ParseResponse(const Bytes& encoded) {
    const std::vector<Field> fields = DecodeRecord(
    ...
    if (response.signature.size() != identity::kCompositeSignatureLen) {
        throw Error("relay-v2 responder signature size is invalid");
    }
    return response;
}

bool EqualBytes(const Bytes& left,
...
Bytes EncodeIdentity(const identity::CompositeKeyPair& keys) {
    const Bytes encoded = identity::encode_composite_identity(
...
Bytes CanonicalizeIdentity(const Bytes& pem_bundle) {
    ...
    identity::CompositePublicKey parsed =
        identity::parse_composite_identity(pem_bundle);
    if (!parsed.valid()) {
        throw Error("relay-v2 composite identity is invalid");
    }
    const Bytes canonical = identity::encode_composite_identity(
...
InitiatorRequest BeginInitiator(
    const HandshakeContext& context,
    const identity::CompositeKeyPair& initiator_identity,
    const Bytes& responder_identity,
    Bytes relay_psk) {
...
ResponderResult Respond(
    ...
    const identity::CompositeKeyPair& responder_identity,
    Bytes relay_psk) {
    WipeBytesOnExit wipe_psk(relay_psk);
