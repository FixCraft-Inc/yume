...
Bytes MutateField(const Bytes& record, std::uint8_t id,
...
Bytes CopyField(const Bytes& from, std::uint8_t from_id,
...
relay::HandshakeContext Context(
    relay::PasswordPolicy policy =
        relay::PasswordPolicy::Required) {
    relay::HandshakeContext context;
    context.channel_kind = relay::ChannelKind::File;
...
Bytes Psk(std::uint8_t byte = 0x5a) {
    return Bytes(relay::kRelayPskBytes, byte);
}
...
struct Identities {
    yume::relay::identity::CompositeKeyPair initiator{
        yume::relay::identity::generate_composite_keypair()};
    yume::relay::identity::CompositeKeyPair responder{
        yume::relay::identity::generate_composite_keypair()};
    yume::relay::identity::CompositeKeyPair other{
        yume::relay::identity::generate_composite_keypair()};
    Bytes initiator_encoded{relay::EncodeIdentity(initiator)};
    Bytes responder_encoded{relay::EncodeIdentity(responder)};
    Bytes other_encoded{relay::EncodeIdentity(other)};
};

void TestParityAndDomainSeparation(const Identities& identities) {
    auto pending = relay::BeginInitiator(
        Context(), identities.initiator, identities.responder_encoded, Psk());
    Check(relay::InspectInitiatorRequest(pending.encoded) == Context(),
    ...
    Check(!pending.encoded.empty() &&
              pending.encoded.size() <= relay::kMaxRecordBytes,
          "request record size is invalid");
    auto responder = relay::Respond(
    ...
    Check(!responder.encoded.empty() &&
              responder.encoded.size() <= relay::kMaxRecordBytes,
          "response record size is invalid");
    auto initiator = relay::CompleteInitiator(
    ...
    relay::SessionSecrets moved = std::move(initiator);
    Check(moved.valid() && !initiator.valid(),
          "SessionSecrets move ownership is invalid");
    auto ratchet = relay::MakeSessionRatchet(
        std::move(moved), yume::relay::ratchet::EndpointRole::Client);
...
void TestPasswordPolicyAndPskBinding(const Identities& identities) {
    ...
    ExpectFailure(
        [&] {
            (void)relay::BeginInitiator(
                Context(relay::PasswordPolicy::NotRequired),
...
void TestRequestTamperAndIdentitySwap(const Identities& identities) {
    ...
    const Bytes pq_tamper = MutateField(
        pending.encoded, 12, yume::relay::identity::kEd25519SignatureLen);
    ...
    const std::size_t padding = relay::kMaxRecordBytes - near_cap.size();
    ...
    Check(near_cap.size() == relay::kMaxRecordBytes,
...
void TestResponseFieldMutations(const Identities& identities) {
    for (std::uint8_t id = 1; id <= 15; ++id) {
        ...
        ExpectFailure(
            [&] {
                (void)relay::CompleteInitiator(
                    std::move(pending.state), mutated);
            },
            "mutated response field was accepted");
    }

    auto pq_pending = relay::BeginInitiator(
        Context(), identities.initiator, identities.responder_encoded, Psk());
    auto pq_responder = relay::Respond(
        pq_pending.encoded, Context(), identities.initiator_encoded,
        identities.responder, Psk());
    const Bytes pq_tamper = MutateField(
        pq_responder.encoded, 15, yume::relay::identity::kEd25519SignatureLen);
    ExpectFailure(
        [&] {
            (void)relay::CompleteInitiator(
...
void TestStrictInputBounds(const Identities& identities) {
    Check(relay::CanonicalizeIdentity(identities.responder_encoded) ==
              identities.responder_encoded,
    ...
    bad.initiator_endpoint_id.assign(
        relay::kMaxEndpointIdBytes + 1U, 'a');
    ExpectFailure(
        [&] {
            (void)relay::BeginInitiator(
                bad, identities.initiator, identities.responder_encoded,
...
int main() {
    static_assert(!std::is_copy_constructible_v<relay::InitiatorState>);
    static_assert(!std::is_copy_assignable_v<relay::InitiatorState>);
    static_assert(std::is_nothrow_move_constructible_v<
                  relay::InitiatorState>);
    static_assert(!std::is_copy_constructible_v<relay::SessionSecrets>);
    static_assert(std::is_nothrow_move_constructible_v<
                  relay::SessionSecrets>);
    ...
    std::cout << "relay handshake test ok\n";
    return 0;
}
