...
std::string safe_component(std::string value) {
    ...
    // The v2 instance key: the first 12 bytes of SHA-256, in hex.
    return identity::sha256_hex(value).substr(0, 24);
}
...
std::vector<std::uint8_t> HistoryStore::load_or_create_key() const {
    ...
    auto decode_key = [&](std::vector<std::uint8_t> encoded_bytes) {
        ...
        std::optional<std::string> decoded = base64_decode(encoded);
        if (!decoded) {
            throw std::runtime_error("relay history key is not canonical base64");
        }
        std::string raw = std::move(*decoded);
    ...
    auto generated = storage::random_bytes(kHistoryKeyBytes);
...
std::string HistoryStore::encrypt_line(const std::string& line) const {
    ...
    auto nonce = storage::random_bytes(kHistoryNonceBytes);
    storage::Bytes plain(line.begin(), line.end());
    BytesWiper plain_wiper{plain};
    auto blob = storage::seal_chacha20(plain, key, nonce);
    std::string packed;
...
std::string HistoryStore::decrypt_line(const std::string& line) const {
    std::optional<std::string> decoded = base64_decode(line);
    if (!decoded) throw std::runtime_error("history line is not canonical base64");
    std::string raw = std::move(*decoded);
    ...
    auto key = load_or_create_key();
    BytesWiper key_wiper{key};
    storage::Bytes nonce(raw.begin(), raw.begin() + kHistoryNonceBytes);
    storage::Bytes blob(raw.begin() + kHistoryNonceBytes, raw.end());
    auto plain = storage::open_chacha20(blob, key, nonce);
    BytesWiper plain_wiper{plain};
