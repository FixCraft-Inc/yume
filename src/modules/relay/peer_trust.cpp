...
std::string CanonicalIdentityFingerprint(
    ...
    identity::Bytes encoded(canonical_identity.begin(),
    ...
    const identity::Bytes reencoded = identity::encode_composite_identity(
        parsed.classical.get(), parsed.pq.get());
    if (reencoded != encoded) {
        throw PeerTrustError(
            "relay peer composite identity is not canonical");
    }
    const std::string fingerprint = identity::composite_fingerprint(parsed);
...
std::string RecordName(std::string_view endpoint_id, RecordKind kind) {
    return identity::sha256_hex(endpoint_id) +
