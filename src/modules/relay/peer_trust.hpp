...
struct PeerTrustConfig {
    ...
    // Exact endpoint ID -> lowercase composite fingerprint, the same YTP/1
    // identity fingerprint yume-setup prints for that key. These values are
    // operator-provided/OOB authorization, not learned state.
