...
#include "modules/relay/peer_trust.hpp"
...
using Bytes = yume::relay::identity::Bytes;
...
relay::PeerTrustConfig Config(
        const std::filesystem::path& directory,
        relay::PeerTrustMode mode = relay::PeerTrustMode::Tofu,
        relay::ExplicitPeerPins pins = {}) {
    relay::PeerTrustConfig config;
...
[[maybe_unused]] void TestInputValidation(const std::filesystem::path& root) {
    Check(relay::IsValidPeerEndpointId(
    ...
    Check(!relay::IsValidPeerEndpointId("../peer"),
          "traversal endpoint ID was accepted");
    Check(!relay::IsValidPeerEndpointId("peer/child"),
          "slash endpoint ID was accepted");
    Check(!relay::IsValidPeerEndpointId("."),
          "dot endpoint ID was accepted");
    Check(!relay::IsValidPeerEndpointId(std::string(256, 'a')),
          "oversized endpoint ID was accepted");

    Check(relay::IsCanonicalCompositeFingerprint(std::string(64, 'a')),
          "canonical fingerprint was rejected");
    Check(!relay::IsCanonicalCompositeFingerprint(std::string(63, 'a')),
          "short fingerprint was accepted");
    Check(!relay::IsCanonicalCompositeFingerprint(std::string(64, 'A')),
          "uppercase fingerprint was accepted as canonical");
    Check(!relay::IsCanonicalCompositeFingerprint(std::string(64, 'z')),
          "non-hex fingerprint was accepted");

    ExpectFailure(
        [] { relay::PeerTrustStore store(Config({})); },
        "empty trust directory was accepted");
    ExpectFailure(
        [] {
            relay::PeerTrustStore store(Config("relative/trust"));
        },
        "relative trust directory was accepted");
    ExpectFailure(
        [&] {
            relay::PeerTrustStore store(
    ...
    ExpectFailure(
        [&] {
            ...
            config.mode = static_cast<relay::PeerTrustMode>(99);
            relay::PeerTrustStore store(std::move(config));
        },
        "invalid trust mode was accepted");
    ExpectFailure(
        [&] {
            relay::PeerTrustStore store(Config(
                root / "invalid-pin", relay::PeerTrustMode::Pinned,
...
std::filesystem::path RecordPath(const std::filesystem::path& directory,
    ...
    return directory /
        (yume::relay::identity::sha256_hex(endpoint_id) + std::string(suffix));
}

std::string Fingerprint(const Bytes& identity) {
    auto parsed = yume::relay::identity::parse_composite_identity(identity);
    Check(parsed.valid(), "test composite identity failed to parse");
    return yume::relay::identity::composite_fingerprint(parsed);
}
...
struct Identities {
    yume::relay::identity::CompositeKeyPair first{
        yume::relay::identity::generate_composite_keypair()};
    yume::relay::identity::CompositeKeyPair second{
        yume::relay::identity::generate_composite_keypair()};
    Bytes first_encoded{yume::relay::identity::encode_composite_identity(
        first.classical.public_key.get(), first.pq.public_key.get())};
    Bytes second_encoded{yume::relay::identity::encode_composite_identity(
...
void TestTofuAndProtectedFiles(const std::filesystem::path& root,
    ...
    relay::PeerTrustStore store(Config(trust_directory));
    ...
    ExpectFailure(
        [&] {
            (void)store.precheck(
                ...
                relay::PeerTrustRequirement::Admin);
        },
...
void TestExplicitAndPinnedPolicy(const std::filesystem::path& root,
    ...
    relay::PeerTrustStore tofu(Config(trust_directory));
    ...
    relay::PeerTrustStore explicit_store(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
    ...
    relay::PeerTrustStore without_config(Config(trust_directory));
    Check(without_config.precheck(
              endpoint, identities.first_encoded,
              relay::PeerTrustRequirement::Admin).explicit_authorized,
    ...
    relay::PeerTrustStore changed_config(Config(
        trust_directory, relay::PeerTrustMode::Pinned,
    ...
    relay::PeerTrustStore orphan_store(Config(orphan_directory));
    ExpectFailure(
    ...
    relay::PeerTrustStore pinned_empty(Config(
        pinned_directory, relay::PeerTrustMode::Pinned));
    ExpectFailure(
    ...
    relay::PeerTrustStore pinned_configured(Config(
        configured_directory, relay::PeerTrustMode::Pinned,
        {{std::string(endpoint), identities.first_fingerprint}}));
...
void TestDirectorySymlinks(const std::filesystem::path& root,
    ...
    relay::PeerTrustStore linked(Config(link));
    ExpectFailure(
    ...
    relay::PeerTrustStore nested_store(Config(nested / "child"));
    ExpectFailure(
        [&] {
    ...
    relay::PeerTrustStore public_store(Config(public_directory));
    ExpectFailure(
        [&] {
    ...
    relay::PeerTrustStore writable_parent_store(
...
void TestConcurrentFirstWriters(const std::filesystem::path& root,
    ...
    relay::PeerTrustStore same_store(Config(same_directory));
    ...
    relay::PeerTrustStore different_store(Config(different_directory));
...
void TestListing(const std::filesystem::path& root,
    ...
    {
        relay::PeerTrustStore empty(Config(trust_directory));
    ...
    relay::PeerTrustStore tofu_store(Config(trust_directory));
    (void)tofu_store.commit_verified(tofu_endpoint, identities.first_encoded);

    relay::PeerTrustStore authorized_store(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
    ...
    Check(listed[0].source == relay::PeerTrustSource::Tofu &&
    ...
    Check(listed[1].source == relay::PeerTrustSource::Explicit &&
    ...
    Check(configured[1].source == relay::PeerTrustSource::Configured,
    ...
    relay::PeerTrustStore conflicting(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
    ...
    relay::PeerTrustStore unseen(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
    ...
    Check(with_unseen[2].endpoint_id == unseen_endpoint &&
              with_unseen[2].source ==
                  relay::PeerTrustSource::Configured &&
...
void TestForget(const std::filesystem::path& root,
    ...
    relay::PeerTrustStore configured_only(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
    ...
    relay::PeerTrustStore authorized(Config(
        trust_directory, relay::PeerTrustMode::Tofu,
        {{std::string(explicit_endpoint), identities.second_fingerprint}}));
    ...
    relay::PeerTrustStore unconfigured(Config(trust_directory));
...
void TestListingRejectsPlantedRecords(const std::filesystem::path& root,
    ...
    const std::string forged_name =
        yume::relay::identity::sha256_hex("7777777777777777777777777777aaaa") + ".pin";
    ...
    const std::string name =
        yume::relay::identity::sha256_hex(endpoint) + ".pin";
