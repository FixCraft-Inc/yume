...
yume::relay::storage::Bytes FromHex(std::string_view hex) {
    if (hex.size() % 2U != 0U) {
    ...
    yume::relay::storage::Bytes result;
...
void write_authenticated_history_record(
    ...
    std::string raw_key = base64_decode(encoded_key).value();
    RelaySecretWiper raw_key_wiper{raw_key};
    assert(raw_key.size() == 32U);

    storage::Bytes key(raw_key.begin(), raw_key.end());
    TestBytesWiper key_wiper{key};
    storage::Bytes plaintext_bytes(plaintext.begin(), plaintext.end());
    TestBytesWiper plaintext_bytes_wiper{plaintext_bytes};
    const auto nonce = storage::random_bytes(12U);
    const auto encrypted =
        storage::seal_chacha20(plaintext_bytes, key, nonce);
    ...
    const std::string encoded = base64_encode(packed);
    {
...
int main() {
    ...
    TempDirectory temp;
    storage::Bytes history_test_key(32U, 0x11U);
    TestBytesWiper history_test_key_wiper{history_test_key};
    const storage::Bytes history_test_nonce(12U, 0x22U);
    storage::Bytes history_test_plain{'h', 'i', 's', 't', 'o', 'r', 'y'};
    TestBytesWiper history_test_plain_wiper{history_test_plain};
    auto history_test_blob = storage::seal_chacha20(
        history_test_plain, history_test_key, history_test_nonce);
    auto history_test_opened = storage::open_chacha20(
    ...
    assert(Throws([&] {
        (void)storage::open_chacha20(
            history_test_blob, history_test_key, history_test_nonce);
    }));
    const storage::Bytes empty_history_plain;
    const auto empty_history_blob = storage::seal_chacha20(
        empty_history_plain, history_test_key, history_test_nonce);
    assert(storage::open_chacha20(
