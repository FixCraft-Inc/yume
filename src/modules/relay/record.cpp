...
void ValidateTypeStreamAndFlags(const Frame& frame,
...
void ValidateSealedFrame(const Frame& sealed) {
    ValidateTypeStreamAndFlags(sealed, kFlagInnerEncrypted);
...
void ValidateOpenedApplication(const Frame& frame) {
    if (frame.header.type != kFrameData || frame.header.stream_id != 0 ||
...
void ValidateControlResponse(Frame& frame) {
    if (frame.header.type != kFrameRekeyAck ||
...
bool IsAllowedFrameType(std::uint8_t type) noexcept {
    return type == kFrameData || type == kFrameRekeyInit ||
           type == kFrameRekeyAck;
}

Bytes EncodeSealedFrame(const Frame& sealed) {
...
Frame DecodeSealedFrame(std::span<const std::uint8_t> encoded) {
...
Bytes SealApplication(ratchet::SessionRatchet& ratchet,
    ...
    Frame frame{
        {static_cast<std::uint32_t>(plaintext.size()), kFrameData, 0, 0},
        std::move(plaintext)};
...
ratchet::OpenResult OpenRecord(
    ...
    std::chrono::steady_clock::time_point now) {
    const Frame sealed = DecodeSealedFrame(encoded);
    ratchet::OpenResult result = ratchet.Open(sealed, now);

    if (sealed.header.type == kFrameData) {
    ...
    } else if (sealed.header.type == kFrameRekeyInit) {
        if (result.application_frame || !result.control_response ||
            result.outbound_rekey_completed) {
