...
std::optional<RelayMessageType> try_relay_message_type(
    ...
    if (value == "file_meta") return RelayMessageType::FileMeta;
    if (value == "file_chunk") return RelayMessageType::FileChunk;
    if (value == "file_done") return RelayMessageType::FileDone;
    if (value == "bytes_meta") return RelayMessageType::BytesMeta;
    if (value == "bytes_chunk") return RelayMessageType::BytesChunk;
    if (value == "bytes_done") return RelayMessageType::BytesDone;
    if (value == "admin_req") return RelayMessageType::AdminRequest;
    if (value == "admin_resp") return RelayMessageType::AdminResponse;
    return std::nullopt;
}
...
RelayMessagePolicyDecision EvaluateTransfer(
    ...
    switch (input.transfer_phase) {
        case RelayTransferPhase::AwaitingMetadata:
    ...
    switch (input.channel_kind) {
        ...
        case ChannelKind::Admin:
            if (input.role == RelayChannelRole::Responder) {
                ...
                if (input.message_type == RelayMessageType::AdminRequest) {
                    return Allow(input.transfer_phase);
                }
                return Reject("admin responder accepts requests only",
                              input.transfer_phase);
            }
            if (input.message_type != RelayMessageType::AdminResponse) {
