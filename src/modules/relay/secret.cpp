...
bool validate_relay_secret_b64(const std::string& relay_secret_b64,
    ...
    // one '='. The shape check reports that rule, and the strict decoder then
    // refuses a second spelling of the same bytes.
    ...
    std::optional<std::string> raw = base64_decode(relay_secret_b64);
    if (!raw) {
        if (error) *error = "relay secret has a noncanonical base64 encoding";
        return false;
    }
    StringWiper raw_wiper{*raw};
