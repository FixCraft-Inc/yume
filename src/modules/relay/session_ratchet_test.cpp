...
void AppendU64(yume::relay::ratchet::Bytes& out, std::uint64_t value) {
    for (int shift = 56; shift >= 0; shift -= 8) {
        out.push_back(static_cast<std::uint8_t>(value >> shift));
    }
}

yume::relay::Frame Envelope(std::uint8_t type,
                               std::uint8_t stream_id,
                               yume::relay::ratchet::SealedFrame sealed) {
    yume::relay::ratchet::Bytes payload;
...
void TestOldEpochApplicationRejectedAfterCommit() {
    ...
    Frame new_plain{{1, kFrameData, 1, 0}, {0x66}};
    ...
    auto stale = old_sender.Encrypt(
        kFrameData, 1, kFlagInnerEncrypted, {0x77}, now);
    assert(Throws([&] {
        (void)server.Open(Envelope(kFrameData, 1, std::move(stale)), now);
    }));
}
...
void TestRekeyPreparationOverlapsWithoutCrossingLimits() {
    ...
    // flight.
    Frame full{{static_cast<std::uint32_t>(
    ...
    assert(old_epoch_data.header.type == kFrameData);
    ...
    Frame extra{{1, kFrameData, 1, 0}, {0xc4}};
    ...
    (void)malicious_old_sender.Encrypt(
        kFrameRekeyInit, 0, kFlagInnerEncrypted, {}, now, false);
    (void)malicious_old_sender.Encrypt(
        kFrameData, 1, kFlagInnerEncrypted, full.payload, now);
    auto over_limit = malicious_old_sender.Encrypt(
        kFrameData, 1, kFlagInnerEncrypted, {0xd5}, now, false);
    assert(Throws([&] {
        (void)server.Open(
            Envelope(kFrameData, 1, std::move(over_limit)), now);
    }));
...
void TestSimultaneousDirectionalRekey() {
    ...
    Frame c2s_warm{{1, kFrameData, 1, 0}, {0xa0}};
    Frame s2c_warm{{1, kFrameData, 2, 0}, {0xb0}};
    (void)server.Open(client.Seal(c2s_warm, now + 3ms), now + 3ms);
    (void)client.Open(server.Seal(s2c_warm, now + 3ms), now + 3ms);

    Frame c2s{{1, kFrameData, 1, 0}, {0xa1}};
    Frame s2c{{1, kFrameData, 2, 0}, {0xb2}};
...
void TestBoundedMultiEpochWindowRaisesUsableBudget() {
    ...
    for (std::uint64_t epoch = 0; epoch <= kWindow; ++epoch) {
        Frame full{{static_cast<std::uint32_t>(payload.size()),
                              kFrameData, 1, 0}, payload};
    ...
    Frame more{{1, kFrameData, 1, 0}, {0x01}};
...
void TestNonContiguousOfferRejected() {
    ...
    Frame skipped{{0, kFrameRekeyInit, 0, 0},
        ratchet::BuildRekeyInit(server.inbound_epoch() + 3,
...
void ExchangeRekey(yume::relay::ratchet::SessionRatchet& sender,
                   yume::relay::ratchet::SessionRatchet& receiver,
...
std::uint64_t SealFixedWorkload(yume::relay::ratchet::SessionRatchet& sender,
                                std::chrono::steady_clock::time_point at) {
    const yume::relay::ratchet::Bytes chunk(64U * 1024U, 0x7A);
    const yume::relay::Frame frame{
        {static_cast<std::uint32_t>(chunk.size()), yume::relay::kFrameData, 1, 0},
        chunk};
...
void TestAckDeadlineClampedInBothDirections() {
    using namespace std::chrono_literals;
    const yume::relay::ratchet::Bytes root(32, 0x51);
    const yume::relay::ratchet::Bytes psk(32, 0x52);
    ...
    {
        yume::relay::ratchet::SessionRatchet client(yume::relay::ratchet::EndpointRole::Client,
                                             root, psk, 8, 8);
        yume::relay::ratchet::SessionRatchet server(yume::relay::ratchet::EndpointRole::Server,
                                             root, psk, 8, 8);
        assert(client.rekey_rtt_estimate().samples == 0);
        (void)client.BeginOutboundRekey(base);
        assert(*client.rekey_deadline() ==
               base + yume::relay::ratchet::kMinRekeyAckDeadline);
        (void)server;
    }
    ...
    // minimum, or ordinary jitter would start closing healthy sessions.
    {
        ...
        assert(estimate.allowance == yume::relay::ratchet::kMinRekeyAckDeadline);
        (void)client.BeginOutboundRekey(base + 1s);
        assert(*client.rekey_deadline() ==
               base + 1s + yume::relay::ratchet::kMinRekeyAckDeadline);
    }
    ...
    // closing the session, but stays bounded by the reviewed cap.
    {
        ...
        assert(estimate.allowance > yume::relay::ratchet::kMinRekeyAckDeadline);
        assert(estimate.allowance < yume::relay::ratchet::kMaxRekeyAckDeadline);
...
void TestGrantedAckDeadlineIsFrozenAtOfferTime() {
    using namespace std::chrono_literals;
    const yume::relay::ratchet::Bytes root(32, 0x61);
    const yume::relay::ratchet::Bytes psk(32, 0x62);
    yume::relay::ratchet::SessionRatchet client(yume::relay::ratchet::EndpointRole::Client,
                                         root, psk, 8, 8);
    yume::relay::ratchet::SessionRatchet server(yume::relay::ratchet::EndpointRole::Server,
                                         root, psk, 8, 8);
...
void TestRttAdaptationNeverWidensEpochLimits() {
    using namespace std::chrono_literals;
    const yume::relay::ratchet::Bytes root(32, 0x71);
    const yume::relay::ratchet::Bytes psk(32, 0x72);
    const auto base = std::chrono::steady_clock::time_point{} + 1s;

    yume::relay::ratchet::SessionRatchet fast(yume::relay::ratchet::EndpointRole::Client,
                                       root, psk, 16, 16);
    yume::relay::ratchet::SessionRatchet fast_peer(yume::relay::ratchet::EndpointRole::Server,
                                            root, psk, 16, 16);
    yume::relay::ratchet::SessionRatchet slow(yume::relay::ratchet::EndpointRole::Client,
                                       root, psk, 16, 16);
    yume::relay::ratchet::SessionRatchet slow_peer(yume::relay::ratchet::EndpointRole::Server,
                                            root, psk, 16, 16);
    ...
    assert(fast_allowance == yume::relay::ratchet::kMinRekeyAckDeadline);
    assert(slow_allowance > fast_allowance);
    assert(slow_allowance <= yume::relay::ratchet::kMaxRekeyAckDeadline);
...
DelayedPathProbe ProbeDelayedPath(std::uint16_t window,
    ...
    const yume::relay::ratchet::Bytes root(32, 0x91);
    const yume::relay::ratchet::Bytes psk(32, 0x92);
    yume::relay::ratchet::SessionRatchet client(yume::relay::ratchet::EndpointRole::Client,
                                         root, psk, window, window);
    yume::relay::ratchet::SessionRatchet server(yume::relay::ratchet::EndpointRole::Server,
                                         root, psk, window, window);
    const yume::relay::ratchet::Bytes payload(frame_bytes, 0x5A);
    const yume::relay::Frame data{
        {static_cast<std::uint32_t>(frame_bytes), yume::relay::kFrameData, 1, 0},
        payload};

    const auto start = Tp{} + 1s;
    const auto deadline = start + rtt * rtt_budget;
    const auto one_way = rtt / 2;
    std::deque<std::pair<Tp, yume::relay::Frame>> to_server;
    std::deque<std::pair<Tp, yume::relay::Frame>> to_client;
