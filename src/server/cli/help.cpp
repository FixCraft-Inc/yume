...
_yumed_complete() {
  ...
  local file_opts="--config --cert --tls_cert --key --tls_key --auth-keys --auth-keys-meta --pq-key --real-index --real-secret-file --filter-geolite --anonym-ca-key --anonym-ca-cert --anonym-sub-key --anonym-sub-cert --federation-auth-key --federation-anonym-ca --keys-add --keys-gen"
  case "$prev" in
    --completion)
