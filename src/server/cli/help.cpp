...
_yumed_complete() {
  ...
  local opts="--help -h --version --credits --config --listen --cert --tls_cert --key --tls_key --auth-keys --auth-keys-meta --threads --reverse-port-min --reverse-port-max --dns-server --proxy --obfs --no-obfs --obfs-secret --obfs-pad-multiple --obfs-jitter-ms --tls-handshake-timeout-ms --max-sessions --accept-rate-limit --egress-mbps --robots-deny --filter-list --filter-geolite --filter-memory-mib --client-filter-mode --egress-filter-mode --packet-egress --packet-tun-name --packet-cidr --packet-mtu --bench --fullbench --full-bench --inner --no-inner --inner-heavy --inner-light --inner-dual --inner-required --hop --no-hop --hop-interval --argon2-memory-budget-kib --argon2-max-jobs --pq-key --pq-auto-generate --use-embedded-master --no-embedded-master --allow-exec --allow-local-ip --control-full --codec-allow --allow-codec --allow-monero-rpc --service-allow --monero-rpc-backend --real --real-index --real-root --real-secret --real-secret-file --anonym --anonym-proof-mode --anonym-api --anonym-token --anonym-ca-key --anonym-ca-cert --anonym-sub-key --anonym-sub-cert --server-name --server-id --relay-enable --relay-disable --directory-enable --directory-disable --operator-keys --federation-enable --federation-auth-key --federation-anonym-ca --peer --cluster-join --cluster-bootstrap --public-node --hide-in-the-crowd --upstream-response --upstream-response-dir --upstream-response-ttl --attach-local --keys-list --keys-add --keys-remove --keys-alias --keys-gen --keys-gen-add --ui --boring --timing --completion --root"
  local file_opts="--config --cert --tls_cert --key --tls_key --auth-keys --auth-keys-meta --pq-key --real-index --real-root --real-secret-file --filter-geolite --anonym-ca-key --anonym-ca-cert --anonym-sub-key --anonym-sub-cert --federation-auth-key --federation-anonym-ca --keys-add --keys-gen"
  case "$prev" in
    --completion)
...
void print_help() {
    std::cout
        ...
        << "  --obfs-secret <string>   Shared HMAC admission secret; the client must\n"
        << "                             match. Required and nonempty under\n"
        << "                             --public-node. Empty is development-only.\n"
        ...
        << "  --argon2-memory-budget-kib <KiB>\n"
        << "                           Aggregate admitted Argon2 handshake memory\n"
        << "                             (default 524288 KiB). Requests that would\n"
        << "                             exceed it are rejected before allocation.\n"
        << "  --argon2-max-jobs <N>    Concurrent admitted Argon2 handshakes\n"
        ...
        << "                             --control-full / --no-inner / --no-obfs;\n"
        << "                             requires --auth-keys and a nonempty\n"
        << "                             --obfs-secret.\n"
