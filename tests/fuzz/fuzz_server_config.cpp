 ...
 *
 * libFuzzer entry for the server configuration loader shared by the GUI and
 * the daemon. The key set is closed, so the contract is that any input either
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
    ...
    std::string pointer;
    try {
        const auto parsed = yume::facade::config_io::parse_server_json(
