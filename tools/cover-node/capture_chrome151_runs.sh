...
if ss -ltn | rg -q ":(${cover_port}|${devtools_port})\\b"; then
    echo 'capture ports 39443 or 39222 are already in use' >&2
    exit 1
fi
...
for run_index in $(seq 1 "$run_count"); do
    ...
    sha256sum -- "$run_dir/netlog.json" "$run_dir/sanitized.json" \
        >"$run_dir/SHA256SUMS"
