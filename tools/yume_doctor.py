...
def _validate_tun_network(value: Any, pointer: str, mtu: int) -> None:
    network = _closed_object(value, pointer,
                             {"addresses", "routes", "local_networks", "peer_networks", "dns"})
    prefixes = {}
    for key in ("routes", "local_networks", "peer_networks"):
        values = network[key]
        path = f"{pointer}/{key}"
        if type(values) is not list or not (0 if key == "routes" else 1) <= len(values) <= 64:
            _fail(path, "invalid prefix array size")
        parsed = [_destination_network(item, f"{path}/{i}") for i, item in enumerate(values)]
        if len(set(parsed)) != len(parsed):
            _fail(path, "duplicate prefix")
        prefixes[key] = parsed
        if mtu < 1280 and any(prefix.version == 6 for prefix in parsed):
            _fail(pointer, "IPv6 requires an MTU of at least 1280")
        if key == "routes" and any(a.overlaps(b) for i, a in enumerate(parsed)
                                   for b in parsed[:i] if a.version == b.version):
            _fail(path, "managed routes must not overlap")

    def addresses(values: Any, path: str, dns: bool) -> list:
        if type(values) is not list or not (0 if dns else 1) <= len(values) <= 16:
            _fail(path, "invalid address array size")
        parsed = []
        for i, value in enumerate(values):
            item = f"{path}/{i}"
            text = _string(value, item, MAX_NETWORK_TEXT_BYTES)
            if "%" in text or (dns and "/" in text) or (not dns and "/" not in text):
                _fail(item, "invalid address syntax")
            try:
                address = ipaddress.ip_interface(text)
            except ValueError:
                _fail(item, "invalid address")
            if text != (str(address.ip) if dns else str(address)):
                _fail(item, "must use canonical address syntax")
            if address.ip.is_loopback or any(address.version == prefix.version and address.ip in prefix
                                             for prefix in NEVER_ALLOWED_NETWORKS):
                _fail(item, "address is not allowed on a TUN")
            if address.version == 6 and mtu < 1280:
                _fail(item, "IPv6 requires an MTU of at least 1280")
            allowed = prefixes["peer_networks" if dns else "local_networks"]
            if not any(address.version == prefix.version and address.ip in prefix for prefix in allowed):
                _fail(item, "address must be within the corresponding packet policy")
            if dns and not any(address.version == prefix.version and address.ip in prefix for prefix in prefixes["routes"]):
                _fail(item, "DNS server needs a managed route")
            if address.ip in parsed:
                _fail(item, "duplicate address")
            parsed.append(address.ip)
        return parsed

    addresses(network["addresses"], f"{pointer}/addresses", False)
    dns = _closed_object(network["dns"], f"{pointer}/dns", {"servers", "domains"})
    servers = addresses(dns["servers"], f"{pointer}/dns/servers", True)
    domains = dns["domains"]
    if type(domains) is not list or len(domains) > 16:
        _fail(f"{pointer}/dns/domains", "invalid DNS routing domain array")
    seen = set()
    for i, value in enumerate(domains):
        item = f"{pointer}/dns/domains/{i}"
        domain = _string(value, item, 253)
        if domain != "." and (not _valid_host(domain) or domain != domain.lower() or
                              all(ch in "0123456789." for ch in domain) or ":" in domain):
            _fail(item, "must be a lowercase DNS routing domain or '.'")
        if domain in seen:
            _fail(item, "duplicate DNS routing domain")
        seen.add(domain)
    if bool(servers) != bool(domains):
...
def _validate_adapters(
    ...
    for index, item in enumerate(value):
        ...
        if kind == "socks5":
            ...
            interface = _string(
                adapter["interface_name"], f"{pointer}/interface_name", 15
            )
            if not _valid_identifier(interface, 15):
            ...
            mtu = _integer(adapter["mtu"], f"{pointer}/mtu", 576, 65535)
            _validate_tun_network(adapter["network"], f"{pointer}/network", mtu)
